...
class TestValidationMetrics(unittest.TestCase):
    ...
    def test_nmse(self):
        ...
        expected = np.linalg.norm(self.gt - self.pred) ** 2 / np.linalg.norm(self.gt) ** 2
        self.assertAlmostEqual(result, expected, places=5)
    ...
    def test_ssim(self):
        ...
        maxval = self.gt.max()
        expected = np.mean([structural_similarity(self.gt[i], self.pred[i], data_range=maxval) for i in range(self.gt.shape[0])])
        self.assertAlmostEqual(result, expected, places=5)

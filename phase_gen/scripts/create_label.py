...
logging.basicConfig(filename="./create_label.log",
                    filemode='w+',
...
def load_data(filename: str, orig_path: str, label_path: str) -> tuple:
    ...
    if not os.path.exists(full_label_path):
        filename_new = "_".join(filename.split("_")[:-1] + ["staple"] + filename.split("_")[-1:])
        full_label_path = os.path.join(label_path, filename_new)
    if not os.path.exists(full_label_path):
        filename_new = "_".join(filename.split("_")[:-1] + ["brainmask"] + filename.split("_")[-1:])
        full_label_path = os.path.join(label_path, filename_new)
...
if __name__ == "__main__":
    ...
    with tqdm(total=len(data_list)) as pbar:
        with concurrent.futures.ProcessPoolExecutor(max_workers=2) as executor:
            futures = [executor.submit(main, filename, orig_path, label_path, save_path) for filename in data_list]
